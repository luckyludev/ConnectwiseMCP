...
- [ ] Open `open_attachment_uploader` in Claude. Paste or drop an approved non-sensitive test image larger than 1 MB and verify the app resizes it locally. Attach it to an approved test Ticket, then to an approved TimeEntry. Verify ConnectWise receives the correct record type, the receipt does not echo base64, and Cloudflare logs contain neither image bytes nor filenames.
- [ ] Test upload rejection for an unsupported MIME type, a spoofed MIME signature, a mismatched extension, an image over the prepared 1 MB cap, and a redirect response. Confirm no URL fetch or automatic write retry occurs.
- [ ] For a ticket upload with optional note text, force the note write to fail after the attachment succeeds. Verify the UI reports the attachment/document ID as partial success instead of claiming rollback.
