...
- [ ] Verify the catalog exactly matches [`legacy-read-surface-classification.md`](legacy-read-surface-classification.md): 38 registered tools, 37 model-visible, and `upload_connectwise_image` app-only. Confirm there is no generic API/path/method/body tool, debug/permission probe, memory/filesystem tool, URL-based upload, unbounded document transfer, or caller-selected profile/host.
...
- [ ] Using the normal OAuth flow, call every write-capable tool with a valid non-mutating argument shape and confirm it is denied for missing `mcp:write` before profile-secret lookup or ConnectWise client creation. Do not perform a live write.
- [ ] Treat write enablement as a separate blocked gate. Before any live write test, add and review an explicit `mcp:write` issuance/eligibility policy, obtain change-window approval, and define operation-specific acceptance and cleanup evidence. The current release must not issue `mcp:write`.
...
- [ ] Use a ConnectWise API member that lacks a tested read permission. Confirm ConnectWise remains the final business-authorization boundary and the Worker returns a sanitized permission denial without broadening the role.
