...
### Method 4: Streamable HTTP MCP (new project)

If your client prefers MCP over HTTP streaming instead of stdio/SSE, use the new entry point in `streamable_http/server.py`:

```bash
# from the repo root
python3 streamable_http/server.py
```

Environment variables (in addition to the ConnectWise creds) for HTTP mode:
- `MCP_SERVER_HOST` (default `0.0.0.0`)
- `MCP_SERVER_PORT` (default `8000`)
- `MCP_STREAM_PATH` (default `/mcp`)

Point your MCP HTTP client at `http://<host>:<port><path>` (e.g., `http://localhost:8000/mcp`).
...
curl -v -X POST http://127.0.0.1:8000/mcp \
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"clientInfo":{"name":"curl","version":"0.0.1"}}}'
```
A successful response returns status 200, `content-type: text/event-stream`, and an `mcp-session-id` header. Use that session ID for follow-up calls by sending it in `MCP-Session-ID`. Example:
```bash
SESSION="<value from mcp-session-id>"
curl -v -X POST http://127.0.0.1:8000/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json,text/event-stream" \
